...
float HermiteSplineLuminanceRolloff(float luminance, float target_white, float max_white, float target_black, float min_black, float nits = 100.f) {
  float luminance_pq = Linear_to_PQ(luminance * (nits / HDR10_MaxWhiteNits));
  float target_white_pq = Linear_to_PQ(target_white * (nits / HDR10_MaxWhiteNits));
  float max_white_pq = Linear_to_PQ(max_white * (nits / HDR10_MaxWhiteNits));
  float target_black_pq = Linear_to_PQ(target_black * (nits / HDR10_MaxWhiteNits));
  float min_black_pq = Linear_to_PQ(min_black * (nits / HDR10_MaxWhiteNits));
  float scaled = HermiteSplineRolloff(luminance_pq, target_white_pq, max_white_pq, target_black_pq, min_black_pq);

  float unpq_scaled = PQ_to_Linear(scaled) * (HDR10_MaxWhiteNits / nits);
  return unpq_scaled;
}
...
float3 HermiteSplineLuminanceRolloff(float3 color, float target_white, float max_white, float target_black, float min_black = 0.f, float nits = 100.f) {
  ...
  float3 new_color = RestoreLuminance(color, new_y);
  return new_color;
}
...
float3 HermiteSplinePerChannelRolloff(float3 input, float target_white, float max_white, float target_black, float min_black = 0.f, float nits = 100.f) {
  float3 input_pq = Linear_to_PQ(input * (nits / HDR10_MaxWhiteNits));
  float target_white_pq = Linear_to_PQ(target_white * (nits / HDR10_MaxWhiteNits));
  float max_white_pq = Linear_to_PQ(max_white * (nits / HDR10_MaxWhiteNits));
  float target_black_pq = Linear_to_PQ(target_black * (nits / HDR10_MaxWhiteNits));
  float min_black_pq = Linear_to_PQ(min_black * (nits / HDR10_MaxWhiteNits));
  ...
  float3 unpq_scaled = PQ_to_Linear(scaled) * (HDR10_MaxWhiteNits / nits);
  return unpq_scaled;
}
