...
float XeGTAO_DepthMIPFilter(float depth0, float depth1, float depth2, float depth3, const GTAOConstants consts)
{
    ...
    const float effectRadius = depthRangeScaleFactor * consts.EffectRadius * consts.RadiusMultiplier;
...
void XeGTAO_PrefilterDepths16x16(uint2 dispatchThreadID, uint2 groupThreadID, Texture2D sourceNDCDepth, SamplerState depthSampler, RWTexture2D<float> outDepth0, RWTexture2D<float> outDepth1, RWTexture2D<float> outDepth2, RWTexture2D<float> outDepth3, RWTexture2D<float> outDepth4, const GTAOConstants consts)
{
    // MIP 0
...
void XeGTAO_Denoise(uint2 pixCoordBase, Texture2D sourceAOTermAndEdges, SamplerState texSampler, RWTexture2D<unorm float4> outputTexture, const uniform bool finalApply, const GTAOConstants consts)
{
