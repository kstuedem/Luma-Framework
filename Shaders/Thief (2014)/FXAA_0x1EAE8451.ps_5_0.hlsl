#include "../Includes/Common.hlsl"
#include "../Includes/DICE.hlsl"

Texture2D<float4> t0 : register(t0);

SamplerState s0_s : register(s0);

cbuffer cb0 : register(b0)
{
  float4 cb0[2];
}

#define cmp

// TODO: verify this works well
void main(
  float2 v0 : TEXCOORD0,
  float4 v1 : TEXCOORD1,
  float4 v2 : TEXCOORD2,
  out float4 o0 : SV_Target0)
{
  float4 r0,r1,r2,r3,r4,r5;
  r0.xyzw = t0.SampleLevel(s0_s, v0.xy, 0).xyzw;
  r1.xyzw = cb0[1].xyxy * float4(0,1,1,0) + v0.xyxy;
  r1.x = t0.SampleLevel(s0_s, r1.xy, 0).w;
  r1.y = t0.SampleLevel(s0_s, r1.zw, 0).w;
  r2.xyzw = cb0[1].xyxy * float4(0,-1,-1,0) + v0.xyxy;
  r1.z = t0.SampleLevel(s0_s, r2.xy, 0).w;
  r1.w = t0.SampleLevel(s0_s, r2.zw, 0).w;
  r2.x = max(r1.x, r0.w);
  r2.y = min(r1.x, r0.w);
  r2.x = max(r2.x, r1.y);
  r2.y = min(r2.y, r1.y);
  r2.z = max(r1.z, r1.w);
  r2.w = min(r1.z, r1.w);
  r2.x = max(r2.z, r2.x);
  r2.y = min(r2.w, r2.y);
  r2.z = 0.165999994 * r2.x;
  r2.x = r2.x + -r2.y;
  r2.y = max(0.0833000019, r2.z);
  r2.y = cmp(r2.x >= r2.y);
  if (r2.y != 0) {
    r2.yz = -cb0[1].xy + v0.xy;
    r2.y = t0.SampleLevel(s0_s, r2.yz, 0).w;
    r2.zw = cb0[1].xy + v0.xy;
    r2.z = t0.SampleLevel(s0_s, r2.zw, 0).w;
    r3.xyzw = cb0[1].xyxy * float4(1,-1,-1,1) + v0.xyxy;
    r2.w = t0.SampleLevel(s0_s, r3.xy, 0).w;
    r3.x = t0.SampleLevel(s0_s, r3.zw, 0).w;
    r3.yz = r1.zw + r1.xy;
    r2.x = 1 / r2.x;
    r3.w = r3.y + r3.z;
    r3.y = r0.w * -2 + r3.y;
    r3.z = r0.w * -2 + r3.z;
    r4.x = r2.w + r2.z;
    r2.w = r2.y + r2.w;
    r4.y = r1.y * -2 + r4.x;
    r2.w = r1.z * -2 + r2.w;
    r2.y = r3.x + r2.y;
    r2.z = r3.x + r2.z;
    r3.x = abs(r3.y) * 2 + abs(r4.y);
    r2.w = abs(r3.z) * 2 + abs(r2.w);
    r3.y = r1.w * -2 + r2.y;
    r2.z = r1.x * -2 + r2.z;
    r3.x = abs(r3.y) + r3.x;
    r2.z = abs(r2.z) + r2.w;
    r2.y = r2.y + r4.x;
    r2.z = cmp(r3.x >= r2.z);
    r2.y = r3.w * 2 + r2.y;
    r1.xz = r2.zz ? r1.xz : r1.yw;
    r1.y = r2.z ? cb0[1].y : cb0[1].x;
    r1.w = r2.y * 0.0833333358 + -r0.w;
    r2.yw = r1.zx + -r0.ww;
    r1.xz = r1.xz + r0.ww;
    r3.x = cmp(abs(r2.y) >= abs(r2.w));
    r2.y = max(abs(r2.y), abs(r2.w));
    r1.y = r3.x ? -r1.y : r1.y;
    r1.w = saturate(abs(r1.w) * r2.x);
    r2.x = asfloat(asint(r2.z) & asint(cb0[1].x));
    r2.w = r2.z ? 0 : cb0[1].y;
    r3.yz = r1.yy * float2(0.5,0.5) + v0.xy;
    r3.y = r2.z ? v0.x : r3.y;
    r3.z = r2.z ? r3.z : v0.y;
    r4.xy = r3.yz + -r2.xw;
    r5.xy = r3.yz + r2.xw;
    r3.y = r1.w * -2 + 3;
    r3.z = t0.SampleLevel(s0_s, r4.xy, 0).w;
    r1.w = r1.w * r1.w;
    r3.w = t0.SampleLevel(s0_s, r5.xy, 0).w;
    r1.x = r3.x ? r1.z : r1.x;
    r1.z = 0.25 * r2.y;
    r2.y = -r1.x * 0.5 + r0.w;
    r1.w = r3.y * r1.w;
    r2.y = cmp(r2.y < 0);
    r3.x = -r1.x * 0.5 + r3.z;
    r3.y = -r1.x * 0.5 + r3.w;
    r3.zw = cmp(abs(r3.xy) >= r1.zz);
    r4.z = -r2.x * 1.5 + r4.x;
    r4.x = r3.z ? r4.x : r4.z;
    r4.w = -r2.w * 1.5 + r4.y;
    r4.z = r3.z ? r4.y : r4.w;
    r4.yw = asfloat(~asint(r3.zw));
    r4.y = asfloat(asint(r4.w) | asint(r4.y));
    r4.w = r2.x * 1.5 + r5.x;
    r5.x = r3.w ? r5.x : r4.w;
    r4.w = r2.w * 1.5 + r5.y;
    r5.z = r3.w ? r5.y : r4.w;
    if (r4.y != 0) {
      if (r3.z == 0) {
        r3.x = t0.SampleLevel(s0_s, r4.xz, 0).w;
      }
      if (r3.w == 0) {
        r3.y = t0.SampleLevel(s0_s, r5.xz, 0).w;
      }
      r4.y = -r1.x * 0.5 + r3.x;
      r3.x = r3.z ? r3.x : r4.y;
      r3.z = -r1.x * 0.5 + r3.y;
      r3.y = r3.w ? r3.y : r3.z;
      r3.zw = cmp(abs(r3.xy) >= r1.zz);
      r4.y = -r2.x * 2 + r4.x;
      r4.x = r3.z ? r4.x : r4.y;
      r4.y = -r2.w * 2 + r4.z;
      r4.z = r3.z ? r4.z : r4.y;
      r4.yw = asfloat(~asint(r3.zw));
      r4.y = asfloat(asint(r4.w) | asint(r4.y));
      r4.w = r2.x * 2 + r5.x;
      r5.x = r3.w ? r5.x : r4.w;
      r4.w = r2.w * 2 + r5.z;
      r5.z = r3.w ? r5.z : r4.w;
      if (r4.y != 0) {
        if (r3.z == 0) {
          r3.x = t0.SampleLevel(s0_s, r4.xz, 0).w;
        }
        if (r3.w == 0) {
          r3.y = t0.SampleLevel(s0_s, r5.xz, 0).w;
        }
        r4.y = -r1.x * 0.5 + r3.x;
        r3.x = r3.z ? r3.x : r4.y;
        r3.z = -r1.x * 0.5 + r3.y;
        r3.y = r3.w ? r3.y : r3.z;
        r3.zw = cmp(abs(r3.xy) >= r1.zz);
        r4.y = -r2.x * 2 + r4.x;
        r4.x = r3.z ? r4.x : r4.y;
        r4.y = -r2.w * 2 + r4.z;
        r4.z = r3.z ? r4.z : r4.y;
        r4.yw = asfloat(~asint(r3.zw));
        r4.y = asfloat(asint(r4.w) | asint(r4.y));
        r4.w = r2.x * 2 + r5.x;
        r5.x = r3.w ? r5.x : r4.w;
        r4.w = r2.w * 2 + r5.z;
        r5.z = r3.w ? r5.z : r4.w;
        if (r4.y != 0) {
          if (r3.z == 0) {
            r3.x = t0.SampleLevel(s0_s, r4.xz, 0).w;
          }
          if (r3.w == 0) {
            r3.y = t0.SampleLevel(s0_s, r5.xz, 0).w;
          }
          r4.y = -r1.x * 0.5 + r3.x;
          r3.x = r3.z ? r3.x : r4.y;
          r3.z = -r1.x * 0.5 + r3.y;
          r3.y = r3.w ? r3.y : r3.z;
          r3.zw = cmp(abs(r3.xy) >= r1.zz);
          r4.y = -r2.x * 2 + r4.x;
          r4.x = r3.z ? r4.x : r4.y;
          r4.y = -r2.w * 2 + r4.z;
          r4.z = r3.z ? r4.z : r4.y;
          r4.yw = asfloat(~asint(r3.zw));
          r4.y = asfloat(asint(r4.w) | asint(r4.y));
          r4.w = r2.x * 2 + r5.x;
          r5.x = r3.w ? r5.x : r4.w;
          r4.w = r2.w * 2 + r5.z;
          r5.z = r3.w ? r5.z : r4.w;
          if (r4.y != 0) {
            if (r3.z == 0) {
              r3.x = t0.SampleLevel(s0_s, r4.xz, 0).w;
            }
            if (r3.w == 0) {
              r3.y = t0.SampleLevel(s0_s, r5.xz, 0).w;
            }
            r4.y = -r1.x * 0.5 + r3.x;
            r3.x = r3.z ? r3.x : r4.y;
            r3.z = -r1.x * 0.5 + r3.y;
            r3.y = r3.w ? r3.y : r3.z;
            r3.zw = cmp(abs(r3.xy) >= r1.zz);
            r4.y = -r2.x * 2 + r4.x;
            r4.x = r3.z ? r4.x : r4.y;
            r4.y = -r2.w * 2 + r4.z;
            r4.z = r3.z ? r4.z : r4.y;
            r4.yw = asfloat(~asint(r3.zw));
            r4.y = asfloat(asint(r4.w) | asint(r4.y));
            r4.w = r2.x * 2 + r5.x;
            r5.x = r3.w ? r5.x : r4.w;
            r4.w = r2.w * 2 + r5.z;
            r5.z = r3.w ? r5.z : r4.w;
            if (r4.y != 0) {
              if (r3.z == 0) {
                r3.x = t0.SampleLevel(s0_s, r4.xz, 0).w;
              }
              if (r3.w == 0) {
                r3.y = t0.SampleLevel(s0_s, r5.xz, 0).w;
              }
              r4.y = -r1.x * 0.5 + r3.x;
              r3.x = r3.z ? r3.x : r4.y;
              r3.z = -r1.x * 0.5 + r3.y;
              r3.y = r3.w ? r3.y : r3.z;
              r3.zw = cmp(abs(r3.xy) >= r1.zz);
              r4.y = -r2.x * 2 + r4.x;
              r4.x = r3.z ? r4.x : r4.y;
              r4.y = -r2.w * 2 + r4.z;
              r4.z = r3.z ? r4.z : r4.y;
              r4.yw = asfloat(~asint(r3.zw));
              r4.y = asfloat(asint(r4.w) | asint(r4.y));
              r4.w = r2.x * 2 + r5.x;
              r5.x = r3.w ? r5.x : r4.w;
              r4.w = r2.w * 2 + r5.z;
              r5.z = r3.w ? r5.z : r4.w;
              if (r4.y != 0) {
                if (r3.z == 0) {
                  r3.x = t0.SampleLevel(s0_s, r4.xz, 0).w;
                }
                if (r3.w == 0) {
                  r3.y = t0.SampleLevel(s0_s, r5.xz, 0).w;
                }
                r4.y = -r1.x * 0.5 + r3.x;
                r3.x = r3.z ? r3.x : r4.y;
                r3.z = -r1.x * 0.5 + r3.y;
                r3.y = r3.w ? r3.y : r3.z;
                r3.zw = cmp(abs(r3.xy) >= r1.zz);
                r4.y = -r2.x * 2 + r4.x;
                r4.x = r3.z ? r4.x : r4.y;
                r4.y = -r2.w * 2 + r4.z;
                r4.z = r3.z ? r4.z : r4.y;
                r4.yw = asfloat(~asint(r3.zw));
                r4.y = asfloat(asint(r4.w) | asint(r4.y));
                r4.w = r2.x * 2 + r5.x;
                r5.x = r3.w ? r5.x : r4.w;
                r4.w = r2.w * 2 + r5.z;
                r5.z = r3.w ? r5.z : r4.w;
                if (r4.y != 0) {
                  if (r3.z == 0) {
                    r3.x = t0.SampleLevel(s0_s, r4.xz, 0).w;
                  }
                  if (r3.w == 0) {
                    r3.y = t0.SampleLevel(s0_s, r5.xz, 0).w;
                  }
                  r4.y = -r1.x * 0.5 + r3.x;
                  r3.x = r3.z ? r3.x : r4.y;
                  r3.z = -r1.x * 0.5 + r3.y;
                  r3.y = r3.w ? r3.y : r3.z;
                  r3.zw = cmp(abs(r3.xy) >= r1.zz);
                  r4.y = -r2.x * 2 + r4.x;
                  r4.x = r3.z ? r4.x : r4.y;
                  r4.y = -r2.w * 2 + r4.z;
                  r4.z = r3.z ? r4.z : r4.y;
                  r4.yw = asfloat(~asint(r3.zw));
                  r4.y = asfloat(asint(r4.w) | asint(r4.y));
                  r4.w = r2.x * 2 + r5.x;
                  r5.x = r3.w ? r5.x : r4.w;
                  r4.w = r2.w * 2 + r5.z;
                  r5.z = r3.w ? r5.z : r4.w;
                  if (r4.y != 0) {
                    if (r3.z == 0) {
                      r3.x = t0.SampleLevel(s0_s, r4.xz, 0).w;
                    }
                    if (r3.w == 0) {
                      r3.y = t0.SampleLevel(s0_s, r5.xz, 0).w;
                    }
                    r4.y = -r1.x * 0.5 + r3.x;
                    r3.x = r3.z ? r3.x : r4.y;
                    r3.z = -r1.x * 0.5 + r3.y;
                    r3.y = r3.w ? r3.y : r3.z;
                    r3.zw = cmp(abs(r3.xy) >= r1.zz);
                    r4.y = -r2.x * 2 + r4.x;
                    r4.x = r3.z ? r4.x : r4.y;
                    r4.y = -r2.w * 2 + r4.z;
                    r4.z = r3.z ? r4.z : r4.y;
                    r4.yw = asfloat(~asint(r3.zw));
                    r4.y = asfloat(asint(r4.w) | asint(r4.y));
                    r4.w = r2.x * 2 + r5.x;
                    r5.x = r3.w ? r5.x : r4.w;
                    r4.w = r2.w * 2 + r5.z;
                    r5.z = r3.w ? r5.z : r4.w;
                    if (r4.y != 0) {
                      if (r3.z == 0) {
                        r3.x = t0.SampleLevel(s0_s, r4.xz, 0).w;
                      }
                      if (r3.w == 0) {
                        r3.y = t0.SampleLevel(s0_s, r5.xz, 0).w;
                      }
                      r4.y = -r1.x * 0.5 + r3.x;
                      r3.x = r3.z ? r3.x : r4.y;
                      r3.z = -r1.x * 0.5 + r3.y;
                      r3.y = r3.w ? r3.y : r3.z;
                      r3.zw = cmp(abs(r3.xy) >= r1.zz);
                      r4.y = -r2.x * 4 + r4.x;
                      r4.x = r3.z ? r4.x : r4.y;
                      r4.y = -r2.w * 4 + r4.z;
                      r4.z = r3.z ? r4.z : r4.y;
                      r4.yw = asfloat(~asint(r3.zw));
                      r4.y = asfloat(asint(r4.w) | asint(r4.y));
                      r4.w = r2.x * 4 + r5.x;
                      r5.x = r3.w ? r5.x : r4.w;
                      r4.w = r2.w * 4 + r5.z;
                      r5.z = r3.w ? r5.z : r4.w;
                      if (r4.y != 0) {
                        if (r3.z == 0) {
                          r3.x = t0.SampleLevel(s0_s, r4.xz, 0).w;
                        }
                        if (r3.w == 0) {
                          r3.y = t0.SampleLevel(s0_s, r5.xz, 0).w;
                        }
                        r4.y = -r1.x * 0.5 + r3.x;
                        r3.x = r3.z ? r3.x : r4.y;
                        r1.x = -r1.x * 0.5 + r3.y;
                        r3.y = r3.w ? r3.y : r1.x;
                        r1.xz = cmp(abs(r3.xy) >= r1.zz);
                        r3.z = -r2.x * 8 + r4.x;
                        r4.x = r1.x ? r4.x : r3.z;
                        r3.z = -r2.w * 8 + r4.z;
                        r4.z = r1.x ? r4.z : r3.z;
                        r1.x = r2.x * 8 + r5.x;
                        r5.x = r1.z ? r5.x : r1.x;
                        r1.x = r2.w * 8 + r5.z;
                        r5.z = r1.z ? r5.z : r1.x;
                      }
                    }
                  }
                }
              }
            }
          }
        }
      }
    }
    r1.x = v0.x + -r4.x;
    r1.z = -v0.x + r5.x;
    r2.x = v0.y + -r4.z;
    r1.x = r2.z ? r1.x : r2.x;
    r2.x = -v0.y + r5.z;
    r1.z = r2.z ? r1.z : r2.x;
    r2.xw = cmp(r3.xy < float2(0,0));
    r3.x = r1.z + r1.x;
    r2.xy = asfloat((asint(r2.xw) != asint(r2.yy)) ? 0xFFFFFFFF : 0);
    r2.w = 1 / r3.x;
    r3.x = cmp(r1.x < r1.z);
    r1.x = min(r1.x, r1.z);
    r1.z = r3.x ? r2.x : r2.y;
    r1.w = r1.w * r1.w;
    r1.x = r1.x * -r2.w + 0.5;
    r1.w = 0.75 * r1.w;
    r1.x = asfloat(asint(r1.x) & asint(r1.z));
    r1.x = max(r1.x, r1.w);
    r1.xy = r1.xx * r1.yy + v0.xy;
    r2.x = r2.z ? v0.x : r1.x;
    r2.y = r2.z ? r1.y : v0.y;
    r0.xyz = t0.SampleLevel(s0_s, r2.xy, 0).xyz;
  }
  o0.xyzw = r0.xyzw;
#if 0 // TODO: Luma - are these additions necessary? Needed by the UI?
  o0.w = saturate(o0.w);
#elif 1
  o0.w = 1.f;
#endif

  bool forceVanillaSDR = ShouldForceSDR(v0.xy);

#if ENABLE_FAKE_HDR // The game doesn't have many bright highlights, the dynamic range is relatively low, this helps alleviate that. Ideally it'd be better to do this before FXAA, to anti alias it better, but whatever
  if (LumaSettings.DisplayMode == 1 && !forceVanillaSDR)
  {
    float normalizationPoint = 0.025; // Found empyrically
    float fakeHDRIntensity = 0.4;
    float saturationBoost = 0.666;
    o0.xyz = FakeHDR(o0.xyz, normalizationPoint, fakeHDRIntensity, saturationBoost);
  }
#endif

  const float paperWhite = LumaSettings.GamePaperWhiteNits / sRGB_WhiteLevelNits;
  const float peakWhite = LumaSettings.PeakWhiteNits / sRGB_WhiteLevelNits;
	DICESettings settings = DefaultDICESettings();
	o0.xyz = DICETonemap(o0.xyz * paperWhite, peakWhite, settings) / paperWhite;

#if POST_PROCESS_SPACE_TYPE == 0 // This is just for testing really, it's not good here as there's a couple optional passes after this (e.g. Vignette), that would need acknowledging of the changed gamma values
  o0.rgb = linear_to_sRGB_gamma(o0.rgb);
#endif
}