...
ApplyPerChannelCorrectionResult ApplyPerChannelCorrectionInternal(
  ...
  const float per_channel_color_max = max(per_channel_color.x, max(per_channel_color.y, per_channel_color.z));
  ...
  float3 final_color = JzAzBz::jzazbzToRgb(float3(
      tonemapped_perceptual.x,
      final_chromas));

  //scale to orig max
  const float final_color_max = max(final_color.x, max(final_color.y, final_color.z));
  final_color *= per_channel_color_max / final_color_max;

  //clamp
  final_color = max(0, final_color);
