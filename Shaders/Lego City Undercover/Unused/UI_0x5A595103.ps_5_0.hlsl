// ---- Created with 3Dmigoto v1.3.16 on Wed Jul 30 00:06:31 2025
...
SamplerState specular_sampler_ss_s : register(s5);
SamplerState surface_sampler_ss_s : register(s6);
SamplerState legoSplatGun_sampler_ss_s : register(s7);
TextureCube<float4> sceneEnvmap_samplerCube : register(t3);
Texture2D<float4> specular_sampler : register(t5);
Texture2D<float4> surface_sampler : register(t6);
  ...
  out float4 o0 : SV_Target0)
{
  ...
  r2.w = r1.w * r0.y;
  r0.y = -r0.y * r1.w + 1;
  r1.w = specular_sampler.Sample(specular_sampler_ss_s, v6.xy).x;
  r1.w = g_MaterialPS.fs_specular_params.y * r1.w;
