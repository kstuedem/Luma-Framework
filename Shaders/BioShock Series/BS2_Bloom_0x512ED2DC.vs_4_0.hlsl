...
{
  float ImageSpace_hlsl_BlurVertexShader00000000000000000000000000000003_2bits : packoffset(c0) = {0};
  ...
  float4 SampleWeights[3] : packoffset(c18);
  float4 PWLConstants : packoffset(c21);
  float PWLThreshold : packoffset(c22);
  float ShadowEdgeDetectThreshold : packoffset(c22.y);
  float4 ColorFill : packoffset(c23);
  float2 LowResTextureDimensions : packoffset(c24);
  float2 DownsizeTextureDimensions : packoffset(c24.z);
}

// Luma: this might only be used when DoF is also enabled, but anyway it's part of the bloom processing
void main(
  float4 v0 : POSITION0,
  float4 v1 : TEXCOORD0,
  float2 v2 : TEXCOORD1,
