// XeGTAO implementation adopted for Burnout Paradise Remastered
//
...
{
  float4 g_Vector4_A : packoffset(c0);
}
...
// FIXME: Should use games true FOV.
#define FOV_Y 47.0

#define TAN_HALF_FOV_Y tan(FOV_Y * XE_GTAO_PI / 180.0 * 0.5)
#define TAN_HALF_FOV_X (TAN_HALF_FOV_Y * LumaSettings.SwapchainSize.x * rcp(LumaSettings.SwapchainSize.y))
...
{
  return 1.0 / (screenDepth * g_Vector4_A.x + g_Vector4_A.y);
}
...
void main_pass_cs(uint2 dtid : SV_DispatchThreadID)
{
    ...
    // smp = g_samplerPointClamp
    XeGTAO_MainPass(dtid, SpatioTemporalNoise(dtid, 0), tex0, smp, ao_term_and_edges);
}
